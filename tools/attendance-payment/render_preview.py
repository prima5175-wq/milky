#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""설치 후 시트 모습 PNG 렌더 (오늘=2026-05-30)."""
import datetime
...
D=datetime.date
# (이름, 등록여부, 금액, 횟수, 시간, 납부, 할인, 등록일, 출석들, 다음등록일)
STUDENTS=[
 ('정채원','결제완료_정상등록',140000,'주1회','60분','월','',D(2026,5,4),
   [D(2026,5,6),D(2026,5,13),D(2026,5,27)], D(2026,6,2)),
 ('강서진','결제완료_정상등록',660000,'주2회','90분','분기','형제할인',D(2026,3,10),
   [D(2026,3,11),D(2026,3,28),D(2026,4,8),D(2026,4,15),D(2026,4,22),D(2026,5,6),D(2026,5,20),D(2026,5,27)],
   D(2026,5,31)),
 ('김태윤','결제완료_정상등록',300000,'주2회','120분','월','',D(2026,5,1),
   [D(2026,5,2),D(2026,5,8),D(2026,5,15),D(2026,5,22),D(2026,5,29)], D(2026,5,30)),
 ('강지후','결제대기 중',990000,'매일반','90분','분기','오픈할인',D(2026,5,11),
...
x0=15; y0=15
d.text((x0*S,y0*S),'설치 후 명단 시트 미리보기  ·  할인(형제5%/오픈20%) · 다음등록일 색경고 · 주차 띠  (오늘 2026-05-30)',font=f_title,fill='#000')
y=y0+38
...
ctext(week_x+WEEKN*WCELL/2,y-14,'◀ 주차 띠(첫 칸부터) ▶',f_small,'#783f04')
ctext(grid_x+GRIDN*GCELL/2,y-14,'◀ 회차 칸 ▶',f_small,'#1f4e79')
y+=RH

for idx,st in enumerate(STUDENTS):
    name,reg,amt,freq,dur,cyc,disc,regd,dates,nextd=st
    ...
    for (nm,w),v in zip(INFO,vals):
        ...
        if nm=='할인' and disc: fc='#ffd966' if disc=='오픈할인' else '#b6d7a8'
        if nm=='다음\n등록일': fc=next_color(nextd)
        box(x,y,w,rows*RH,fc)
        ctext(x+w/2,y+rows*RH/2-7,v,f_cell,'#b06000' if (nm=='금액' and disc) else '#222')
        x+=w
...
ly=y+6
d.text((x0*S,ly*S),'할인: 형제할인 5% · 오픈할인 20% (정상금액에서 자동 차감)   |   다음등록일: 3일전 노랑·1일전 주황·당일/지남 빨강   |   주차 초록=출석/빨강=결석',font=f_small,fill='#555')

# 등록회차 무지개 색상표
ly2=ly+26
d.text((x0*S,ly2*S),'▼ 등록회차 드롭다운 색(무지개 순)',font=f_cell,fill='#000')
cy=ly2+20; cw=150; ch=22
for i,opt in enumerate(PLAN_OPTS):
    col=i%7; rr=i//7
    bx=x0+col*cw; by=cy+rr*(ch+4)
    box(bx,by,cw-6,ch,plan_color(opt)); ctext(bx+(cw-6)/2,by+5,opt,f_small,'#222')
