...
- **`src` 를 비워두면 "준비 중" 자리**가 표시돼요. 자료를 아직 못 구했을 때 유용해요

```javascript
media: [{ type: 'video', caption: '개원식 스케치 (촬영 예정)' }]
```

현재 STEP 4·STEP 20 에 예시 슬롯이 들어가 있어요.
...
실제 휴대폰(폭 760px 이하)에서는 토글이 자동으로 숨겨져요.
