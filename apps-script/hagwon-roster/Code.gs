 ...
 *     · 분기납 → 아래로 2줄 자동 삽입(총 3줄) · 월납 → 1줄
 *     · 매일반(월납) → 2줄 · 매일반(분기납) → 6줄
 *     · 시간별 색: 60분 분홍(주1회 전용) · 90분 초록 · 120분 노랑
 *     · 60분 수업은 주1회에서만 운영(주2회·주3회·매일반은 90분/120분만)
...
const CODE_VERSION = 'v52 (2026-07-17) 매일반 월/분기 구분 + 60분 옵션(주2·주3·매일반) 삭제';
...
function planOptions_() {
  ...
  ['주1회', '주2회', '주3회'].forEach(f =>
    DURS_BY_FREQ[f].forEach(d =>
      ['월', '분기'].forEach(c => opts.push(`${f} ${d} ${c}`))));
  ['90분', '120분'].forEach(d =>
    ['월', '분기'].forEach(c => opts.push(`매일반 ${d} ${c}`)));
  return opts;
}
...
function setupSheet() {
  ...
  // 머리글 안내
  sh.getRange(1, COL_PLAN).setNote('등록회차를 고르면 회차 칸이 자동 생성됩니다.\n분기납=3줄, 월납=1줄, 매일반 월납=2줄, 매일반 분기납=6줄.\n색: 60분 분홍(주1회 전용)·90분 초록·120분 노랑.');
...
function buildPriceSheet_(ss) {
  ...
  // ※ 60분 수업은 주1회에서만 운영(주2회·주3회·매일반은 60분 미운영)
  const base = [
    ['주1회','60분',140000,390000],['주1회','90분',180000,480000],['주1회','120분',220000,630000],
    ['주2회','90분',240000,660000],['주2회','120분',300000,810000],
    ['주3회','90분',340000,840000],['주3회','120분',400000,990000],
    ['매일반','90분','',990000],['매일반','120분','',1110000],
  ];
  const rows = [];
...
function parsePlan_(text) {
  ...
  // 납부주기: 글자에 '월'/'분기'가 명시되어 있으면 그걸 따르고,
  // 옛 형식처럼 표시가 없으면 매일반=분기(예전 기본값), 그 외=월로 처리(안전한 기본값 유지)
  const cycle = t.indexOf('월') >= 0 ? '월' : (t.indexOf('분기') >= 0 ? '분기' : (daily ? '분기' : '월'));
  const perMonth = daily ? GRID_COLS : FREQ_PERMONTH[freq]; // 매일반=15칸/줄
  const rows = daily ? (cycle === '분기' ? 6 : 2) : (cycle === '분기' ? 3 : 1); // 매일반: 분기=6줄(3개월) · 월=2줄(1개월)
  const months = cycle === '분기' ? 3 : 1;        // 다음등록일 계산용
...
function migrateGridTo15() {
  ...
  renumber_(sh);
  ui.alert('정리 완료',
    '기존 데이터를 15칸 배치로 옮겼어요. 매일반은 플랜(월/분기)에 맞게 다시 펴졌습니다.\n' +
